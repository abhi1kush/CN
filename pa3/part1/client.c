...
int main(int argc, char *argv[])
{
	    int sockfd, portnum, n;
		    ...
		    char inputbuf[MAX_INPUT_SIZE];
		        if (argc < 4) {
				       fprintf(stderr,"usage %s <my-ip-address> <server-ip-addr> <server-port>\n", argv[0]);
				              exit(0);
					          }
			    ...
			    portnum = atoi(argv[3]);
				    ...
				    if (sockfd < 0) 
				        ...
				        /* Bind to local private address */
				        bzero((char *) &client_addr, sizeof(client_addr));
					    client_addr.sin_family = AF_INET;
					        client_addr.sin_port = 0;
						    if(!inet_aton(argv[1], &client_addr.sin_addr))
							          {
									  	fprintf(stderr, "ERROR invalid client IP address\n");
											exit(1);
											      }
